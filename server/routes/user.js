...
/* Get RENT LIST*/
router.get("/:userId/rents",async(req,res)=>{
try{
  const { userId } =req.params
  //console.log(userId)
  const rents = await Booking.find({ hostId: userId }).populate("customerId hostId listingId")
  //console.log(rents)
  res.status(202).json(rents)
}
catch(err){
  console.log(err)
  res.status(404).json({ message: "Can not find rented listings!", error: err.message })
}
})
